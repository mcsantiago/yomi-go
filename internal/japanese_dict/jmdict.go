...
type JmdictSource struct {
	...
	// not from an actual phrase in at language. Most commonly used to
	// indicate "waseieigo".
...
func LoadJmdictMap(reader io.Reader) (map[string][]JmdictEntry, error) {
	var dict Jmdict
	_, err := parseDict(reader, &dict, true)

	// Convert the Jmdict to a map
	jmdictMap := make(map[string][]JmdictEntry)
	for _, entry := range dict.Entries {
		for _, k := range entry.Kanji {
			jmdictMap[k.Expression] = append(jmdictMap[k.Expression], entry)
		}
	}

	return jmdictMap, err
}
